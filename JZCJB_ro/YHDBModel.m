...
@implementation YHDBModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [YHDBModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    YHDBHelper *yhDB = [YHDBHelper shareInstance];
    [yhDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    FMDatabase *db = [FMDatabase databaseWithPath:[YHDBHelper dbPath]];
    if (![db open]) {
        NSLog(@"数据库打开失败!");
        return NO;
    }
...
{
    //判断是否是YHBaseModel的子类
    for (YHDBModel *model in array) {
        if (![model isKindOfClass:[YHDBModel class]]) {
            return NO;
        }
    }
    ...
    YHDBHelper *yhDB = [YHDBHelper shareInstance];
    // 如果要支持事务
    [yhDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (YHDBModel *model in array) {
...
+ (NSArray *)findAll
{
    ...
    [yhDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            YHDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
